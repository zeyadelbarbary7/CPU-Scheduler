...
void priorityPreemptive(){
    ...
    do{
        ...
        if(!minProcesses.empty()){
            if (startTimes.find(minProcesses[0].getName()) == startTimes.end()) {
                startTimes[minProcesses[0].getName()] = time; // Record the start time
            }

            minProcesses[0].setBurstTime(minProcesses[0].getBurstTime()-1); // Decrease the burst time of the process that is executing
            if(minProcesses[0].getBurstTime()==0){ // If the process has finished executing
                // cout<<minProcesses[0].getName()<<"\t"<<minProcesses[0].getArrivalTime()<<"\t"<<minProcesses[0].getBurstTime()<<"\t"<<minProcesses[0].getPriority()<<"\t\t"<<startTimes[minProcesses[0].getName()]<<"\t";
                waitingTime = time+1 -minProcesses[0].getArrivalTime()-minProcesses[0].getOriginalBurstTime();
                // cout<<time<<"\t"<<waitingTime<<endl;
                totalWaitingTime+=waitingTime;
                minProcesses.erase(minProcesses.begin()); // remove the process that has been executed
            }

            if(minProcesses[0].getName()!=currentProcess||time==0){
                cout<<time<<minProcesses[0].getName()<<"|";
            }   
            currentProcess = minProcesses[0].getName();
        }
        time++;
